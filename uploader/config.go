...
type Config struct {
	...
	BatchQuerySize       int              `toml:"batch-query-size"`           // 0 - disabled. Butch size for check metrics in database (for index or tagged table), if not found in local cache instead of simple insert
}
